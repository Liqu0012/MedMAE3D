...
from rope.deit.models_v2 import vit_models, Layer_scale_init_Block, Attention
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ndim = x.ndim
    if freqs_cis.shape[-2:] == x.shape[-2:]:  # 匹配最后两维
        shape = [1] * (ndim - 2) + list(freqs_cis.shape)
    elif freqs_cis.shape[-3:] == x.shape[-3:]:  # 匹配最后三维
        shape = [1] * (ndim - 3) + list(freqs_cis.shape)
    else:
        raise ValueError(f"Unmatched shapes for broadcasting: freqs_cis.shape = {freqs_cis.shape}, x.shape = {x.shape}")
